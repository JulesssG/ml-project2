...
def first():
    ...
    try:
        for i, trace in enumerate(st):
            trace.write(f"data/downloads/train/10/{i}.slist", format="SLIST")
...
def second():
    ...
    try:
        for i, trace in enumerate(st):
            trace.write(f"data/downloads/train/11/{i}.slist", format="SLIST")
    except FileNotFoundError:
        print("Wrong setup, file not found error")
...
def third():
    ...
    try:
        for i, trace in enumerate(st):
            trace.write(f"data/downloads/train/12/{i}.slist", format="SLIST")
    except FileNotFoundError:
        print("Wrong setup, file not found error")
